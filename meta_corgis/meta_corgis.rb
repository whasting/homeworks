...
class MetaCorgiSnacks
  def initialize(snack_box, box_id)
    @snack_box = snack_box
    @box_id = box_id
    method_names = @snack_box.methods
    method_names.grep(/^get_(.*)_info$/) do
      MetaCorgiSnacks.define_snack $1
    end
    method_names.grep(/^get_(.*)_tastiness$/) do
      MetaCorgiSnacks.define_snack $1
    end
  end
  ...
  #   if method_name.start_with?("kibble", "treat", "bone")
  #     info_text = "get_#{method_name}_info"
  #     tastiness_text = "get_#{method_name}_tastiness"
  #     info = @snack_box.send(info_text.to_sym, @box_id)
  #     tastiness = @snack_box.send(tastiness_text.to_sym, @box_id)
  #     result = "Bone: #{info}: #{tastiness} "
  #     tastiness > 30 ? "* #{result}" : result
  #   else
  #     super
